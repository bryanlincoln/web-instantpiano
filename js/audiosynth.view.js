var WHITE_KEY_WIDTH = (screen.width - 40) / 21;
var WHITE_BLACK_DIFF = 5; // diferença entre o tamanho das teclas pretas e brancas /2
...
function AudioSynthView() {
	...
	var fnCreateKeyboard = function(keyboardElement) {
			...
			for(var n in notes) {
				if(n[2] != 'b') {
					...
					if(n.length>1) {
						...
						thisKey.style.left = (WHITE_KEY_WIDTH * (iWhite - 1)) + (WHITE_KEY_WIDTH/2 + WHITE_BLACK_DIFF) + 'px';
					} else {
						thisKey.className = 'white key';
						thisKey.style.left = WHITE_KEY_WIDTH * iWhite + 'px';
						iWhite++;
					}
		...
		visualKeyboard.style.width = iWhite * WHITE_KEY_WIDTH + 'px';
	...
	var fnPlayKeyboard = function(e) {
		...
		if(keyboard[e.keyCode]) {
			...
			window.record(note, octaveModifier);
		} else {
			return false;	
		}
    ...
    window.fnPlaySong = fnPlaySong;
	window.fnChangeOctave = fnChangeOctave;
}